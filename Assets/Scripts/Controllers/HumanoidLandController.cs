...
public class HumanoidLandController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // if (_playerMoveInput.x > 0f || _playerMoveInput.y > 0f)
        // {
...
//#endif
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.CompareTag("second"))
        {
            audioManager.StopVoiceInHead();
            audioManager.PlayVoiceInHead(audioManager.secondArea);
            Destroy(col.GameObject());
        }
        else if (col.gameObject.CompareTag("third"))
        {
            audioManager.StopVoiceInHead();
            audioManager.PlayVoiceInHead(audioManager.thirdArea);
            Destroy(col.GameObject());

        }
        else if (col.gameObject.CompareTag("fourth"))
        {
            audioManager.StopVoiceInHead();
            audioManager.PlayVoiceInHead(audioManager.fourthArea);
            Destroy(col.GameObject());
        }
        else if (col.gameObject.CompareTag("sixth"))
        {
            audioManager.StopVoiceInHead();
            audioManager.PlayVoiceInHead(audioManager.almostThere);
            Destroy(col.GameObject());
        }
